...
def process_transcript_with_claude(api_key, transcript_text):
    client = anthropic.Anthropic(api_key=api_key)
    
    try:
        logging.info("Preparing to send request to Claude API")
        message = client.messages.create(
            model="claude-3-sonnet-20240229",
            max_tokens=4000,
            temperature=0,
            system="You are an expert at analyzing transcripts and creating detailed markdown documents.",
            messages=[
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": f"""Please analyze the following transcript and create a detailed markdown document. 
...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.log.emit("Claude API analysis completed")

            markdown_file = os.path.abspath(f"{os.path.splitext(self.output_file)[0]}_analysis.md")
            self.log.emit(f"Markdown file generated: {markdown_file}")

            self.progress_update.emit(100)
            self.output.emit(f"Process completed successfully.\nTranscript saved to: {self.output_file}\nMarkdown analysis saved to: {markdown_file}")
        except Exception as e:
    ...
    def process_with_claude(self):
        try:
            ...
            self.log.emit("Sending transcript to Claude API for analysis...")
            markdown_output = process_transcript_with_claude(self.api_key, transcript_text)
            self.log.emit("Received analysis from Claude API")
            
            self.log.emit("Generating markdown file...")
            markdown_file = os.path.abspath(f"{os.path.splitext(self.output_file)[0]}_analysis.md")
