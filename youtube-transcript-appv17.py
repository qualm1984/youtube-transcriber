...
def process_transcript_with_claude(api_key, transcript_text):
    ...
    message_content = f"""Please analyze the following transcript and create a detailed markdown document. 
...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.log.emit("Processing transcript with Claude API...")
            markdown_output = self.process_with_claude()
            self.log.emit("Claude API processing completed")

            markdown_file = os.path.abspath(f"{video_title}_analysis.md")
            with open(markdown_file, 'w', encoding='utf-8') as f:
                f.write(markdown_output)
            self.log.emit(f"Created markdown file: {markdown_file}")

            self.progress_update.emit(100)
            self.output.emit(f"Process completed. Transcript saved to: {self.output_file}\nAnalysis saved to: {markdown_file}")
    ...
    def process_with_claude(self):
        try:
            ...
            return process_transcript_with_claude(self.api_key, transcript_text)
        except Exception as e:
            self.log.emit(f"Error in process_with_claude: {str(e)}")
