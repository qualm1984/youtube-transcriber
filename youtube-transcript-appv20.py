...
class WorkerThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.markdown_file = os.path.abspath(f"{os.path.splitext(self.output_file)[0]}_analysis.md")
            with open(self.markdown_file, 'w', encoding='utf-8') as f:
                f.write("")  # Create an empty file
            self.log.emit(f"Created empty markdown file: {self.markdown_file}")
...
class YouTubeTranscriptApp(QWidget):
    ...
    def on_process_finished(self):
        ...
        self.log_message("Process finished. You can now close the application or process another video.")
